...
vector_data* vector_get_data(cvector vec) { return &((vector_data*)vec)[-1]; }
...
void vector_free(cvector vec) { free(vector_get_data(vec)); }

vec_size_t vector_size(cvector vec) { return vector_get_data(vec)->length; }

vec_size_t vector_get_alloc(cvector vec) { return vector_get_data(vec)->alloc; }
...
void* _vector_add(cvector* vec_addr, vec_type_t type_size)
{
...
void* _vector_insert(cvector* vec_addr, vec_type_t type_size, vec_size_t pos)
{
	vector_data* v_data = vector_get_data(*vec_addr);
...
void vector_pop(cvector vec) { --vector_get_data(vec)->length; }
